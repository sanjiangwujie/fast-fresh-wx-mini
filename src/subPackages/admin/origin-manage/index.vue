<template>
  <view class="page">
    <view class="header">
      <text class="header-title">产地管理</text>
    ...
    <scroll-view class="scroll-view" scroll-y>
      <view class="list">
        <view class="list-item" v-for="item in origins" :key="item.id">
    ...
    <view class="modal" v-if="showModal" @click="handleCloseModal">
      <view class="modal-content" @click.stop>
        <view class="modal-header">
          <text class="modal-title">{{ editingItem ? '编辑产地' : '新增产地' }}</text>
          <text class="modal-close" @click="handleCloseModal">×</text>
        </view>
        <view class="modal-body">
          <view class="form-item">
            <text class="form-label">产地名称</text>
            <input class="form-input" v-model="formData.name" placeholder="请输入产地名称" />
          </view>
          <view class="form-item">
            <text class="form-label">产地分类</text>
            <input class="form-input" v-model="formData.category_name" placeholder="如：国内、进口" />
          </view>
        </view>
...
import { getOrigins, createOrigin, updateOrigin, deleteOrigin } from "@/api/origin";
...
export default {
  setup() {
    ...
    const editingItem = ref<Origins | null>(null);
    const formData = ref({
      name: "",
      category_name: "",
    });
    ...
    const handleSubmit = async () => {
      ...
      try {
        if (editingItem.value) {
          await updateOrigin(editingItem.value.id, formData.value);
          uni.showToast({ title: "更新成功", icon: "success" });
        } else {
          await createOrigin(formData.value);
