<template>
  <view class="page">
    <view class="header">
      <text class="header-title">果农管理</text>
    ...
    <scroll-view class="scroll-view" scroll-y>
      <view class="list">
        <view class="list-item" v-for="item in farmers" :key="item.id">
          <view class="item-content">
            <view class="item-info">
              <text class="item-name">{{ item.nickname || item.phone }}</text>
              <text class="item-phone">{{ item.phone }}</text>
              <text class="item-farmer" v-if="item.farmer">果农名称: {{ item.farmer.name || '未设置' }}</text>
            </view>
          </view>
        </view>
      </view>
    ...
    <view class="modal" v-if="showModal" @click="handleCloseModal">
      <view class="modal-content" @click.stop>
        <view class="modal-header">
          <text class="modal-title">新增果农</text>
        ...
        <view class="modal-body">
          ...
          <view class="form-item">
            <text class="form-label">果农名称</text>
            <input class="form-input" v-model="formData.farmer_name" placeholder="请输入果农名称" />
          </view>
          <view class="form-item">
            <text class="form-label">初始密码（可选）</text>
            <input class="form-input" v-model="formData.password" placeholder="不设置则无密码" type="password" />
          </view>
        </view>
...
import { getFarmers, createFarmerAccount } from "@/api/user";
import type { Users } from "@/types/graphql";

export default {
  setup() {
    const farmers = ref<Array<Users & { farmer?: any }>>([]);
    ...
    const loadFarmers = async () => {
      try {
        ...
        console.error("加载果农列表失败:", error);
        uni.showToast({ title: "加载失败", icon: "none" });
      }
    };

    const handleCreate = () => {
      formData.value = { phone: "", nickname: "", farmer_name: "", password: "" };
    ...
    const handleSubmit = async () => {
      ...
      try {
        await createFarmerAccount({
          ...
          farmer_name: formData.value.farmer_name || undefined,
